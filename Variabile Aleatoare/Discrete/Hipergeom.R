...
I_DiscHiperParams <- function(id, input) {
    tagList(
        # parametru n Nr Bile NEGRE
        column(4, sliderInput(paste(id, "_m", sep =""), "m: (nr bile negre) cele pe care le dorim", min = 0, max = 100, 30, 1)),
        # parametru n Nr Bile ALBE
        column(4, sliderInput(paste(id, "_n", sep =""), "n: (nr bile albe)", min = 1, max = 100, 30, 1)),
        # parametru k Nr Bile Extrase
        column(4, renderUI({
            sliderInput(paste(id, "_k", sep =""), "k: (nr bile extrase)", min = 0, max = input$hiper_m + input$hiper_n, input$hiper_m, 1)
        }))
    )
}
IO_DiscHiperProbA <- function(id, input){
    tagList(
        # input a
        column(6, renderUI({
            sliderInput(paste(id, "_a", sep =""), "P(x <= a):", min = 0, max = min(input$hiper_m, input$hiper_k), input$hiper_m, 1)
        })),
        # output
...
IO_DiscHiperProbB <- function(id, input){
    tagList(
        # input b
        column(6, renderUI({
            sliderInput(paste(id, "_b", sep =""),"P(x >= b):", min = 0, max = min(input$hiper_m, input$hiper_k), 1, 1)
        })), 
        # output 
        column(6, br(), br(), renderPrint({
...
IO_DiscHiperProbCD <- function(id, input){
    tagList(
        column(6,
               # input c
               column(4, renderUI({
                   numericInput(paste(id, "_c", sep =""),"x >= c:", min = 0, max = min(input$hiper_m, input$hiper_k), value = 1)
               })),
               # input d
               column(4, renderUI({
                   numericInput(paste(id, "_d", sep =""),"x <= d:",min = 0, max = min(input$hiper_m, input$hiper_k), value = 1)
               }))    
        ),
        # output
        column(6, br(), renderPrint({
            if (input$hiper_c < input$hiper_d)
...
O_DiscHiperFuncMasa <- function(input){
    renderPlot({
...
O_DiscHiperFuncRep <- function(input){
    tagList(
        ...
        renderPlot({
            ...
            if (input$hiper_c <= input$hiper_d){
                ...
                legendCD = paste("P(", input$hiper_c,  "<= X <=",  input$hiper_d, ")")
            } else {
                colFinalCD = rep("red", input$hiper_m + 1)
...
DiscHipergeom <- function(id_rep, input, output, label = "DiscHipergeom"){
    tagList(
        fluidRow(
            HTML(
                "<h4 style='padding:15px; margin:0;'>Fie o urna cu 'm' bile negre si 'n' bile albe.
                            Consideram ca v.a X este data de numarul de bile negre extrase atunic cand din urna extragem 'k' bile.</h4>"
            )
        ), hr(),
        fluidRow(
            I_DiscHiperParams(id_rep, input)
        ), hr(),
        # CALC PROB P(X <= a)
        fluidRow(
            IO_DiscHiperProbA(id_rep, input)
        ),
        # CALC PROB P(X >= b)
        fluidRow(
            IO_DiscHiperProbB(id_rep, input)
        ),
        # CALC PROB P(c <= X <= d)
        fluidRow(
            IO_DiscHiperProbCD(id_rep, input)
        ),
        # FUNCTIA DE MASA: P(X = x)
        fluidRow(
            O_DiscHiperFuncMasa(input)
        ),
        # FUNCTIA DE REPARTITIE
        fluidRow(
            O_DiscHiperFuncRep(input)
        )
    )
}
